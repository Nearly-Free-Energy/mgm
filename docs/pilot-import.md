...
This foundation supports review and bounded import metadata in a fresh MGM database. It does not connect to a source system or retain source files or raw source identifiers. There is no browser route for importing data. The standalone SQL is in supabase/pilot/20260924_mgm_review_import_essentials.sql. Do not run the standard migration chain until 00016 and the remaining legacy migrations have been reviewed for this deployment.
...
`household_devices.effective_from` is inclusive and `effective_to` is exclusive; `NULL` means current. Existing assignments are backfilled from their creation date. The original primary-meter uniqueness index remains in place. A later migration must add a separate effective-dated replacement history model before multiple historical primary-meter assignments can be stored and used for period-crossing reviews.
...
No real source data has been requested or copied. A future real pilot import remains pending until the operator grants source access and confirms the source-to-MBE field mapping, data minimization basis, pilot organization and microgrid, effective dates for meter replacements, and baseline period. The operator must review a dry-run reconciliation and resolve conflicts before a separately approved import. Keep all source access and import execution server-side; never commit source extracts or credentials.
