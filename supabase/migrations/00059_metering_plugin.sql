...
-- (00056) enforces its own trusted-name whitelist in the body, which still
-- rejects every name except organization-directory/community-management.
-- Redefine it here with the expanded set so Metering is actually toggleable
-- through Settings. Body is otherwise byte-identical to 00056. The core
...
BEGIN
  ...
  IF _plugin_name NOT IN ('organization-directory', 'community-management', 'metering') THEN
